...
def _text_width(text, style):
    ...
    try:
        ...
        text_width_text = run(
            ["inkscape", "--pipe", "--query-width", "--query-id", "label"],
            input=text_query_file,
            encoding="utf8",
            stdout=PIPE,
            stderr=DEVNULL,
        ).stdout
    ...
    return {
        ...
        "shape": {
            ...
            "coords": (
                translate_x,
                translate_y,
                rect_width + translate_x,
                rect_height + translate_y,
            ),
        },
    }
