...
async def config_aio_app(app, settings):
    ...
    app["oauth_providers_by_name"] = {
        provider["name"]: provider for provider in settings["oauth_providers"]
    }
    app["authorization"] = settings.get("authorization", True)

    max_age = int(timedelta(days=30).total_seconds())
    storage = aiohttp_session.cookie_storage.EncryptedCookieStorage(
        settings["aiosession_encryption_key"], max_age=max_age
    )
...
async def show_identity(request, writer, identity=sentinel):
    ...
    if identity:
        ...
        w(
            Tag(
                "a",
                class_="waves-effect waves-light btn",
            ...
            for provider in request.app["oauth_providers"]:
                w(
                    Tag(
                        "a",
                        class_="waves-effect waves-light btn",
                        href=request.app.router["oauth_login"]
                        .url_for(provider=provider["name"])
                        .with_query(return_to=request.path),
                    ),
...
def ensure_authorized_event(handler):
    async def ensure_authorized_inner(request, event):
        ...
        return await ensure_authorized_handler(
            request, allowed_principals, functools.partial(handler, event=event)
        )
