...
from trackers.base import RateLimitingSemaphore, Tracker, print_tracker, stream_store
...
async def config(app, settings):
    app["spot.session"] = session = aiohttp.ClientSession(
        connector=aiohttp.TCPConnector(limit=1), raise_for_status=True
    )
...
async def monitor_feed(app, tracker, feed_id, start: datetime, end: datetime):
    try:
        ...
        store_path = Path(
            app["trackers.settings"]["cache_path"],
            "spot",
            f"{feed_id}-{start.isoformat()}",
        )

        with stream_store(store_path, tracker.logger) as (
            loaded_messages,
            write_messages,
        ):
            ...
            while True:
                try:
                    ...
                    if now > start:
                        if tracker.points:
                            last = (
                                (tracker.points[-1]["time"] + timedelta(seconds=1))
                                .astimezone(timezone.utc)
                                .replace(tzinfo=None)
                            )

                        new_messages = await get_new_messages(
                            app, tracker, feed_id, start, end, last, seen_ids
                        )
                        if new_messages:
...
async def get_new_messages(
    app, tracker, feed_id, start: datetime, end: datetime, last: datetime, seen_ids
):
    if last:
    ...
    while need_to_query_more:
        ...
        for error in tree.findall("errors/error"):
            ...
            if error["code"] == "E-0195":
                ...
                level = logging.ERROR
            tracker.logger.log(
                msg=f"{error['text']} ({error['code']}): {error['description']}",
                level=level,
            )
        ...
        messages = [
            xml_to_dict(item) for item in tree.findall("feedMessageResponse/messages/message")
        ]
        ...
        new_points.append(
            {
                ...
                "server_time": cast(datetime, message["sever_time"])
                .astimezone()
                .replace(tzinfo=None),
...
async def main():
    ...
    async with config(app, settings):
        tracker = await start_tracker(
            app,
            "foobar",
            "0Iejmjaumysr3dcqnMb96E3TnpaqlB60L",
            datetime(2023, 4, 8),
            None,
        )
