...
async def get_config(app, settings, args):
    ...
    async with aiohttp.ClientSession() as session:
        race_page_text = await get(session, f'http://trackleaders.com/{trackleaders_event_name}f.php')
        riders_list_text = await get(session, f'http://trackleaders.com/spot/{trackleaders_event_name}/sortlist.php')
        route_text = await get(session, f'http://trackleaders.com/spot/{trackleaders_event_name}/route.js')
        markers_text = await get(session, f'http://trackleaders.com/spot/{trackleaders_event_name}/checkgen.js')
    ...
    route_points = [
        [float(str(subnode)) for subnode in point_node]
        for point_node in route_point_nodes.items
    ]
    event.routes = [{'original_points': route_points}, ]
        ...
        if isinstance(node, ExprStatement):
            if isinstance(node.expr, Assign):
                ...
                if id.startswith('markercp'):
                    markers_details[id]['location'] = [float(str(loc_node)) for loc_node in node.expr.right.identifier.node.args.items[0].items]
            if isinstance(node.expr, FunctionCall):
                id = node.expr.identifier.node.value
                if id.startswith('markercp') and node.expr.identifier.identifier.value == 'bindTooltip':
                    markers_details[id]['text'] = node.expr.args.items[0].value[4:-5]
    config['markers'] = [
        {
            'marker_text': details['text'],
            'position': {'lat': details['location'][0], 'lng': details['location'][1], },
        }
...
async def monitor_feed(app, tracker, name, event):
    try:
        while True:
            try:
                new_all_points = await get_points(tracker.logger, app['trackleaders.session'], name, event)
