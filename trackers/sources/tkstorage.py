...
from trackers.analyse import Point, distance
...
async def config(app, settings):
    app["tkstorage.points_received_observables"] = points_received_observables = defaultdict(
        partial(Observable, "tkstorage.points_received")
    )
    ...
    app["tkstorage.sms_gateway_status_changed"] = sms_gateway_status_changed = Observable(
        "tkstorage.sms_gateway_status_changed"
    )
    ...
    if isinstance(app, web.Application):
        ...
        sms_gateway_status_changed.subscribe(
            partial(send_sms_gateway_status_changed_to_admin_ws, app)
        )

        app.router.add_route(
            "GET",
            "/tkstorage_admin/tkstorage_websocket",
            handler=admin_ws,
            name="tkstorage_admin_ws",
        )
        app.router.add_route(
            "GET",
            "/tk/{id}",
...
class TrackerObjects(object):
    ...
    def _update_active_tracker(self):
        self.values["active"] = [
            tracker for tracker, count in self.active_trackers.items() if count
        ]
        self.values["prev_active"] = [
            tracker for tracker, count in self.active_trackers.items() if not count
        ]
    ...
    async def apply_config(self, urgent):
        if self.desired_configs:
            ...
            if commands and self.app["tkstorage.config"]:
                await self.app["tkstorage.send_queue"].put(
                    {"id": self.tk_id, "commands": commands, "urgent": urgent}
                )
            return commands
                            ...
                            if isinstance(msg, dict):
                                if "trackers" in msg:
                                    ...
                                    for tracker in app["tkstorage.trackers"]:
                                        app["tkstorage.points_received_observables"][
                                            tracker
                                        ] = Observable(f"tkstorage.points_received.{tracker}")
                                if "sms_gateway_status" in msg:
                                    app["tkstorage.sms_gateway_status"] = msg["sms_gateway_status"]
                                    await app["tkstorage.sms_gateway_status_changed"](
                                        msg["sms_gateway_status"]
                                    )
                            if isinstance(msg, list):
                                ...
                                for item in msg:
                                    try:
                                        ...
                                        logger.error(
                                            f"Error in msg_item_to_point: msg={msg!r} \n {e}"
                                        )
                                        point = None
                                ...
                                new_points = [
                                    consume_config_from_point(app, point) for point in new_points
                                ]
                                ...
                                await values_changed(
                                    {
                                        tk_id: trackers_objects[tk_id].values
                                        for tk_id in trackers_values_changed
                                    }
                                )

                                for tk_id, tracker_points in groupby(
                                    sorted(new_points, key=tk_id_key), key=tk_id_key
                                ):
...
routetrack_time_re = re.compile(
    r"Notice: System has entered routetrack function for (\d*) hour\(s\)."
)
...
def parse_date_time(date_raw: str, time_raw: str, server_time: datetime):
    ...
    if date_raw == "000000":
        ...
        utc_time00 = datetime(
            utc_server_time.year,
            utc_server_time.month,
            utc_server_time.day,
            hour,
            minutes,
            sec,
        ...
        utc_time = min(
            ...
            utc_timeP1,
            utc_timeM1,
            key=lambda t: abs((utc_server_time - t).total_seconds()),
        )
    else:
    ...
    if routetrack:
        ...
        if "rupload" in c and "rsampling" in c:
            if c["rupload"] == c["rsampling"]:
                ...
                config_texts.append(
                    f'Routetrack {t} {c["rupload"]} sec upload, {c["rsampling"]} sec sample'
                )
        else:
...
class TKStorageTracker(Tracker):
    ...
    async def start(cls, app, tracker_name, id, start, end, config={}):
        ...
        if end:
            tracker.complete_on_end_time_reached_task = run_forget_task(
                tracker.complete_on_end_time_reached()
            )
        else:
    ...
    def use_point(self, point):
        ...
        if time_between(time, self.start, self.end):
            return True
        if time_between(time, self.config_read_start, self.end) or time_between(
            server_time, self.config_read_start, self.end
        ):
...
async def admin_ws(request):
    ...
    with contextlib.ExitStack() as exit_stack:
        try:
            ...
            await web_app.message_to_multiple_wss(
                app,
                [ws],
                {
                    "values": {
                        objects.tk_id: objects.values for objects in trackers_objects.values()
                    },
                ...
                if msg.type == WSMsgType.text:
                    try:
                        ...
                        if "config" in data:
                            ...
                            tracker_objects.add_desired_config(
                                "admin_console", data["config"], rank=100
                            )
...
async def send_values_changed_to_admin_ws(app, values_changed):
    await web_app.message_to_multiple_wss(
        app, app["tkstorage.admin_ws_sessions"], {"changed_values": values_changed}
    )


async def send_trackers_changed_to_admin_ws(app, trackers):
    await web_app.message_to_multiple_wss(
        app, app["tkstorage.admin_ws_sessions"], {"trackers": trackers}
    )
...
async def send_sms_gateway_status_changed_to_admin_ws(app, sms_gateway_status):
    await web_app.message_to_multiple_wss(
        app,
        app["tkstorage.admin_ws_sessions"],
        {"sms_gateway_status": sms_gateway_status},
    )
...
async def main():
    ...
    async with config(app, settings):
        tracker = await TKStorageTracker.start(app, "gary", "TK05", datetime(2021, 4, 10, 6), None)
        print_tracker(tracker)
