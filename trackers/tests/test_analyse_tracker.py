...
def filter_keys(items, keys_to_keep=None, keys_to_remove=None):
    ...
    if keys_to_keep:
        return [
            {key: value for key, value in item.items() if key in keys_to_keep} for item in items
        ]
    if keys_to_remove:
        return [
...
class TestAnalyseTracker(asynctest.TestCase):
    ...
    async def test_with_route(self):
        ...
        tracker.completed.set_result(None)
        analyse_tracker = await AnalyseTracker.start(
            tracker, d("2017/01/01 05:00:00"), event_routes
        )
        await analyse_tracker.complete()
    ...
    async def test_stop(self):
        ...
        t1 = datetime.now()
        await tracker.new_points(({"time": t1, "position": (-26.300822, 28.049444, 1800)},))
    ...
    async def test_with_circular_route(self):
        ...
        tracker.completed.set_result(None)
        analyse_tracker = await AnalyseTracker.start(
            tracker, d("2017/01/01 01:00:00"), event_routes
        )
        await analyse_tracker.complete()
    ...
    async def test_pre_post(self):
        ...
        await tracker.new_points(
            (
                ...
                {
                    "time": d("2017/01/01 02:01:00"),
                    "position": (-26.302245, 28.051139, 1800),
                },
                {
                    "time": d("2017/01/01 02:30:00"),
                    "position": (-27.280315, 27.969365, 1800),
                },
                {
                    "time": d("2017/01/01 02:31:00"),
                    "position": (-27.282870, 27.970620, 1800),
                },
                # During
                {
                ...
                # Post
                {
                    "time": d("2017/01/01 05:02:00"),
                    "position": (-27.280315, 27.969365, 1800),
                },
                {
                    "time": d("2017/01/01 05:03:00"),
                    "position": (-27.282870, 27.970620, 1800),
                },
            )
        )
        ...
        points = filter_keys(
            analyse_tracker.pre_post_tracker.points, keys_to_keep=("track_id", "status")
        )
        print_points(points)
        self.assertSequenceEqual(
            points,
