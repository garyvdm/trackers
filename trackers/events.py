...
async def load_events(
    app,
    tree_reader,
    new_event_observable=Observable("new_event"),
    removed_event_observable=Observable("removed_event"),
):
...
class Event(object):
    def __init__(self, app, name, config=None, routes=None):
        ...
        self.rider_pre_post_new_values_observable = Observable(
            f"event.{name}.rider_pre_post_new_values"
        )
        self.rider_blocked_list_update_observable = Observable(
            f"event.{name}.rider_blocked_list_update"
        )
        self.rider_off_route_blocked_list_update_observable = Observable(
            f"event.{name}.rider_off_route_blocked_list_update"
        )
        self.rider_pre_post_blocked_list_update_observable = Observable(
            f"event.{name}.rider_pre_post_blocked_list_update"
        )
    ...
    async def save(
        ...
        config_bytes = yaml.dump(
            self.config, default_flow_style=False, Dumper=YamlEventDumper
        ).encode()
        ...
        if save_routes:
            ...
            routes_bytes = yaml.dump(
                routes, default_flow_style=False, Dumper=YamlEventDumper
            ).encode()
    ...
    def admin_allowed_principals(self):
        return tuple(chain(self.app["config"].get("admin", ()), self.config.get("admin", ())))
    ...
    async def _start_trackers(self, analyse):
        ...
        tree_reader = (
            TreeReader(self.app["trackers.data_repo"], treeish=self.git_hash)
            if self.git_hash
            else None
        )
        ...
        if analyse and not has_static_analyse:
            ...
            find_closest_cache_dir = os.path.join(
                self.app["trackers.settings"]["cache_path"], "find_closest"
            )
            os.makedirs(find_closest_cache_dir, exist_ok=True)
            if self.routes:
                find_closest_cache = PersistedFuncCache(
                    os.path.join(find_closest_cache_dir, f"2-{self.routes_hash}")
                )
        ...
        if has_static:
            for rider in self.config["riders"]:
                rider_name = rider["name"]
                tracker = await start_implicit_static_event_tracker(
                    self, rider_name, "source", tree_reader
                )
            ...
            for rider in self.config["riders"]:
                ...
                for tracker in rider_trackers:
                    ...
                    start_fut = asyncio.ensure_future(
                        start_tracker(self.app, self, rider_name, tracker, start, end)
                    )
        ...
        for rider in self.config["riders"]:
            ...
            objects.combined_tracker = tracker = await Combined.start(
                f"combined.{rider_name}",
                tuple(chain((objects.data_tracker,), objects.source_trackers)),
            )
            if replay:
            ...
            if analyse and rider.get("type", "rider") == "rider":
                if has_static_analyse:
                    ...
                    objects.analyse_tracker = tracker = await AnalyseTracker.start(
                        ...
                        self.event_start,
                        analyse_routes,
                        find_closest_cache=find_closest_cache,
                        processing_lock=self.app["analyse_processing_lock"],
                    )
                    objects.off_route_tracker = await index_and_hash_tracker(
                        tracker.off_route_tracker
                    )
                    objects.pre_post_tracker = await index_and_hash_tracker(
                        tracker.pre_post_tracker
                    )
                    await self.on_rider_pre_post_new_points(
                        rider["name"],
                        objects.pre_post_tracker,
                        objects.pre_post_tracker.points,
                    )
                    objects.pre_post_tracker.new_points_observable.subscribe(
                        partial(self.on_rider_pre_post_new_points, rider["name"])
                    )
                    tracker.not_pre_post_observable.subscribe(
                        partial(self.on_rider_not_pre_post, rider["name"])
                    )

                objects.off_route_blocked_list = BlockedList.from_tracker(
                    objects.off_route_tracker,
                    entire_block=not is_live,
                ...
                objects.pre_post_blocked_list = BlockedList.from_tracker(
                    objects.pre_post_tracker,
                    entire_block=not is_live,
                    new_update_callbacks=(
                        partial(
                            self.rider_pre_post_blocked_list_update_observable,
                            self,
                            rider["name"],
                        ),
                    ),
            ...
            tracker.new_points_observable.subscribe(
                partial(self.on_rider_new_points, rider["name"])
            )
            tracker.reset_points_observable.subscribe(
                partial(self.on_rider_reset_points, rider["name"])
            )
            ...
            objects.blocked_list = BlockedList.from_tracker(
                ...
                entire_block=not is_live,
                new_update_callbacks=(
                    partial(self.rider_blocked_list_update_observable, self, rider["name"]),
                ),
            )
        ...
        if is_live:
            ...
            self.not_live_complete_trackers_task = asyncio.ensure_future(
                self.not_live_complete_trackers()
            )
    ...
    async def not_live_complete_trackers(self):
        await asyncio.wait(
            [rider_objs.tracker.completed for rider_objs in self.riders_objects.values()]
        )
    ...
    async def on_rider_off_route_update(self, rider_name, blocked_list, update):
        ...
        self.new_points.set()
        await self.rider_off_route_blocked_list_update_observable(
            self, rider_name, blocked_list, update
        )
    ...
    async def on_rider_pre_post_new_points(self, rider_name, tracker, new_points):
        if new_points:
            ...
            for point in new_points:
                ...
                if point_is_newer:
                    has_values_update, values_update = spy(
                        ((k, v) for k, v in point.items() if k in self.pre_post_update_main_keys)
                    )
    ...
    def rider_sort_key_func(self, riders_predicted_points, rider_name):
        ...
        dist_on_route = riders_predicted_points.get(rider_name, {}).get(
            "dist_route"
        ) or rider_values.get("dist_route", 0)
        ...
        while True:
            ...
            with suppress(asyncio.TimeoutError):
                await asyncio.wait_for(
                    self.new_points.wait(),
                    predicted_update_interval - batch_update_interval,
                )
            try:
                await self.batch_update(
                    self.riders_updated,
                    self.riders_off_route_updated,
                    self.riders_pre_post_updated,
                )
            except asyncio.CancelledError:
                raise
            except Exception:
    ...
    async def batch_update(self, riders_updated, riders_off_route_updated, riders_pre_post_updated):
        ...
        riders_predicted_points = {
            rider_objects.rider_name: rider_objects.analyse_tracker.get_predicted_position(time)
            or {}
            for rider_objects in self.riders_objects.values()
            if isinstance(rider_objects.analyse_tracker, AnalyseTracker)
        }
        ...
        if leader_objects.analyse_tracker:
            for point in leader_objects.analyse_tracker.points:
                if "dist_route" in point:
                    going_forward = (
                        point["dist_route"] > last_point["dist_route"] if last_point else True
                    )
                    if going_forward:
        ...
        if leader_points:
            ...
            for rider_name in rider_names_sorted[1:]:
                leader_time_diff = self.get_leader_time_diff(
                    rider_name, riders_predicted_points, leader, leader_points, time
                )
        ...
        self.riders_predicted_points = {
            key: value for key, value in riders_predicted_points.items() if value
        }
        await self.batch_update_observable(
            self,
            time,
            riders_updated,
            riders_off_route_updated,
            riders_pre_post_updated,
        )

    def get_leader_time_diff(
        self, rider_name, riders_predicted_points, leader, leader_points, time
    ):
    ...
    async def convert_to_static(self, tree_writer):
        try:
            ...
            for rider in self.config["riders"]:
                rider_name = rider["name"]
                tracker = await Combined.start(
                    rider_name, self.riders_objects[rider_name].source_trackers
                )
            ...
            for rider in self.config["riders"]:
                ...
                for tracker, sub_tracker_type in trackers:
                    if tracker:
                        await tracker.complete()
                        path = os.path.join(
                            "events", self.name, "static", rider_name, sub_tracker_type
                        )
                        tree_writer.set_data(
                            path, msgpack.dumps(tracker.points, default=json_encode)
                        )
