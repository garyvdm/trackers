...
from numpy import arccos, cross, deg2rad, dot, rad2deg, seterr
from numpy.linalg import norm
from nvector import interpolate, lat_lon2n_E, n_E2lat_lon, n_EB_E2p_EB_E, unit
...
class AnalyseTracker(Tracker):
    ...
    async def start(
        ...
        self.process_initial_points_fut = asyncio.ensure_future(
            self.on_new_points(self.org_tracker, self.org_tracker.points)
        )
    ...
    async def on_new_points(self, tracker, new_points):
        ...
        async with self.processing_lock:
            ...
            for i, point in enumerate(new_points):
                ...
                self.pre_post = pre_post = self.finished or (
                    self.analyse_start_time and self.analyse_start_time > point["time"]
                )
                if "position" in point:
                    ...
                    if not pre_post:
                        if self.prev_route_dist_time:
                            ...
                            max_travel_dist = (
                                300000 * max(time_from_prev_route_dist.total_seconds(), 20) / 3600
                            )
                        elif self.analyse_start_time:
                            time_from_start = point["time"] - self.analyse_start_time
                            max_travel_dist = (
                                300000 * max(time_from_start.total_seconds(), 20) / 3600
                            )
                        else:
                    ...
                    if closest:
                        ...
                        self.going_forward = (
                            not self.prev_route_dist
                        ) or route_dist > self.prev_route_dist

                        if on_route and "elevation" in closest.route:
                            point["route_elevation"] = round(
                                route_elevation(closest.route, route_dist)
                            )
                            if (
                                len(point["position"]) > 2
                                and abs(point["route_elevation"] - point["position"][2]) > 500
                            ):
                        ...
                        if (
                            closest.route_i == 0
                            and abs(route_dist - last_route_point.distance) < 100
                        ):
                    ...
                    if self.prev_point_with_position:
                        ...
                        point["time_from_last"] = time_from_last = (
                            point["time"] - prev_point["time"]
                        )
                        if "server_time" in point and "server_time" in prev_point:
                            point["server_time_from_last"] = (
                                point["server_time"] - prev_point["server_time"]
                            )
                    ...
                    if (
                        on_route
                        and self.prev_on_route
                        and closest.route_i == self.prev_closest.route_i
                    ):
                        dist_from_last = abs(
                    ...
                    if time and dist_from_last:
                        ...
                        if (
                            ...
                            and i == last_point_i
                            and closest
                            and closest.route_i == 0
                            and abs(route_dist - last_route_point.distance) < 2000
                        ):
                            # This is for when they are near to the finish, and the tracker turns off too soon.
                            seconds_to_finish = abs(route_dist - last_route_point.distance) / (
                                dist_from_last / seconds
                            )
                            est_finish_time = point["time"] + timedelta(seconds=seconds_to_finish)
                            self.do_est_finish_fut = asyncio.ensure_future(
                                self.do_est_finish(est_finish_time)
                            )

                    if not pre_post:
                        if not on_route or (
                            not self.going_forward and speed_from_last and speed_from_last > 3
                        ):
                            # self.logger.info('off_route')
                            if (
                                not self.is_off_route
                                and self.prev_point_with_position
                                and self.prev_point_with_position["track_id"]
                                == self.current_track_id
                            ):
                                new_off_route_points.append(
                                    {
                                        "position": self.prev_point_with_position["position"],
                                        "track_id": self.off_route_track_id,
                                    }
                                )
                    ...
                    point["track_id"] = (
                        self.pre_post_track_id if pre_post else self.current_track_id
                    )
                ...
                if i % 10 == 9 or is_last_point:
                    ...
                    if log_time_delta >= 1 or (is_last_point and did_slow_log):
                        self.logger.info(
                            "{}/{} ({:.1f}%) points analysed at {:.2f} points/second.".format(
                                i + 1,
                                len(new_points),
                                (i + 1) / len(new_points) * 100,
                                (i - log_i) / log_time_delta,
                            )
                        )
                        log_time = now
                        log_i = i
    ...
    def get_predicted_position(self, time):
        ...
        if (
            not self.pre_post
            and pp
            and time - pp["time"] < self.track_break_time
            and pp.get("speed_from_last", 0) > 3
            and self.prev_on_route
...
def get_analyse_route(org_route):
    ...
    route["point_pairs"] = [
        get_point_pair_precalc(*point_pair) for point_pair in pairs(route_points)
    ]
    ...
    if route.get("simplified_points_indexes"):
        ...
        logging.info(
            "No pre-calculated simplified_points. Please run process_event_routes on event for faster start up."
        )
        if not route.get("split_at_dist"):
            ...
            simplified_points = ramer_douglas_peucker_sections(
                route_points, 500, route["split_at_dist"], route["split_point_range"]
            )

    route["simplfied_point_pairs"] = [
        get_point_pair_precalc(*point_pair) for point_pair in pairs(simplified_points)
    ]
    logger.debug(
        "Route points: {}, simplified points: {}, distance: {}".format(
            len(route_points),
            len(route["simplfied_point_pairs"]),
            route_points[-1].distance,
        )
    )
    return route
...
def ramer_douglas_peucker_sections(points, epsilon, split_at_dist, split_point_range):
    ...
    for dist in split_at_dist:
        ...
        simplified_points_section = ramer_douglas_peucker(
            points[last_index : closest_index + 1], epsilon
        )
...
find_closest_point_pair_routes_result = collections.namedtuple(
    "closest_point_pair_route", ("route_i", "route", "point_pair", "dist", "point")
)


def find_closest_point_pair_routes_cache_key(
    routes,
    to_point,
    min_search_complex_dist,
    prev_closest_route_i,
    break_out_dist,
    prev_dist,
    max_travel_dist,
):
...
def find_closest_point_pair_routes(
    routes,
    to_point,
    min_search_complex_dist,
    prev_closest_route_i,
    ...
    if len_routes == 1:
        ...
        for route_i, route in enumerate(routes):
            if route_i not in special_routes:
                raw_result = find_closest_point_pair_route(
                    route, to_point, prev_dist, max_travel_dist
                )
                if raw_result:
                    results.append(
                        find_closest_point_pair_routes_result(route_i, route, *raw_result)
                    )
...
find_closest_point_pair_result = collections.namedtuple(
    "closest_point_pair", ("point_pair", "dist", "point")
)
...
def find_closest_point_pair_route(route, to_point, prev_dist, max_travel_dist):
    if max_travel_dist:
        ...
        if route["main"]:
            ...
            get_point_distance = (
                lambda point: point.distance * route["dist_factor"] + route["start_distance"]
            )

        test_point_pair = (
            lambda point_pair: get_point_distance(point_pair[1]) > min_route_dist
            and get_point_distance(point_pair[0]) <= max_route_dist
        )
    else:
        test_point_pair = lambda point_pair: True

    simplified_c_points = (
        find_closest_point_pair_result(
    ...
    route_point_pairs_filtered = chain.from_iterable(
        (
            route_point_pairs[
                simplified_c_point.point_pair[0].index : simplified_c_point.point_pair[1].index + 1
            ]
...
def route_distance_no_adjust(route, closest):
    ...
    return round(
        prev_route_point.distance + distance(prev_route_point, closest.point) + start_distance
    )
...
def find_c_point_from_precalc(to_point, point1, point2, c12, p1h, p2h, dp1p2):
    ...
    if sutable_c is not None:
        ...
        c_dist, c_point = min(
            ((distance(to_point, p), p) for p in (point1, point2)), key=itemgetter(0)
        )
...
def get_equal_spaced_points(points, dist_between_points, start_dist=0, round_digits=6):
    ...
    for point in points[1:]:
        ...
        while point_dist_remaining > dist_between_points:
            ...
            new_point_nv = interpolate(
                (last_point.nv, point.nv),
                (point_distance - point_dist_remaining) / point_distance,
            )
...
def move_along_route(route, dist):
    for i, (point1, point2) in enumerate(pairs(route)):
        dist_between = (
            ...
            if isinstance(point1, IndexedPoint) and isinstance(point2, IndexedPoint)
            else distance(point1, point2)
        )
